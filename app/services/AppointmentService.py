...
class AppointmentService:
    ...
    def update_appointment_status(self, appointment_id, **kwargs):
        ...
        try:
            ...
            raise CustomError(f"Erreur lors de la récupération de l'utilisateur lié au rendez-vous: {e}", 500) from e
        ...
        try:
            ...
            raise CustomError(f"Erreur lors de la récupération de la prestation liée au rendez-vous: {e}", 500) from e
        ...
        try:
            ...
            if new_status in [AppointmentStatus.CONFIRMED, AppointmentStatus.CANCELLED]:
                try:
                    ...
                    raise CustomError(f"Erreur lors de l'envoi de l'email de statut pour le RDV {appointment.id}: {e}", 500) from e

            return updated_appointment

        except Exception as e:
            raise CustomError(f"Erreur lors de la mise à jour du status du rendez-vous: {e}", 500) from e
