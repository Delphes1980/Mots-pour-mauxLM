...
class TestAppointmentServiceSimple(BaseTest):
    """Tests pour AppointmentService sans mocks - utilise la vraie DB"""
    ...
    def test_get_appointment_by_prestation(self):
    ...
    def test_get_appointment_by_user(self):
    ...
    def test_get_appointment_by_user_and_prestation_success(self):
    ...
    def test_appointment_relationships(self):
        """Test que les relations avec User et Prestation fonctionnent"""
        appointment = self.appointment_service.create_appointment(
            message="Test relations",
            user_id=self.test_user.id,
            prestation_id=self.test_prestation.id
        )
        
        # Vérifier que l'appointment a bien les bonnes relations
        self.assertEqual(appointment.user_id, self.test_user.id)
        self.assertEqual(appointment.prestation_id, self.test_prestation.id)
        
        # Vérifier que les objets liés peuvent être récupérés
        retrieved_appointment = self.appointment_service.get_appointment_by_id(appointment.id)
        self.assertEqual(retrieved_appointment.user_id, self.test_user.id)
        self.assertEqual(retrieved_appointment.prestation_id, self.test_prestation.id)

    def test_appointment_message_validation(self):
        """Test validation complète du message"""
        # Message valide
        valid_messages = [
            "Message court",
            "Message de longueur moyenne avec plus de détails",
            "A" * 500,  # Exactement 500 caractères
    ...
    def test_multiple_appointments_same_user_prestation(self):
        """Test création de plusieurs rendez-vous pour même utilisateur/prestation"""
        appointment1 = self.appointment_service.create_appointment(
            message="Premier rendez-vous",
            user_id=self.test_user.id,
            prestation_id=self.test_prestation.id
