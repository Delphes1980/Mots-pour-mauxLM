...
class TestReview(unittest.TestCase):
    ...
    def test_missing_required_fields(self):
        with self.assertRaises(ValueError):
            Review(text=None, rating=5, user=self.user)
        with self.assertRaises(ValueError):
            Review(text='Nice', rating=None, user=self.user)
        with self.assertRaises(ValueError):
            Review(text='Nice', rating=5, user=None)

    def test_invalid_text_type_and_length(self):
        with self.assertRaises(ValueError):
            Review(text='', rating=5, user=self.user)
        with self.assertRaises(TypeError):
            Review(text=123, rating=5, user=self.user)
        with self.assertRaises(ValueError):
            Review(text='A', rating=5, user=self.user)
        with self.assertRaises(ValueError):
            Review(text='A'*501, rating=5, user=self.user)

    def test_invalid_rating_type_and_range(self):
        with self.assertRaises(TypeError):
            Review(text='Nice', rating='bad', user=self.user)
        with self.assertRaises(ValueError):
            Review(text='Nice', rating=0, user=self.user)
        with self.assertRaises(ValueError):
            Review(text='Nice', rating=6, user=self.user)

    def test_rating_boundaries(self):
        # Test limites valides
        review1 = Review(text="Good", rating=1, user=self.user)
        review5 = Review(text="Excellent", rating=5, user=self.user2)
    ...
    def test_invalid_user_type(self):
        with self.assertRaises(TypeError):
            Review(text="Nice", rating=5, user="not_a_user")
    ...
    def test_text_whitespace_handling(self):
        review = Review(text="  Great place!  ", rating=5, user=self.user)
        self.assertEqual(review.text, "Great place!")

    def test_user_assignment(self):
        review = Review(text="Nice place", rating=4, user=self.user)
        self.assertEqual(review.user, self.user)
    ...
    def test_multiple_reviews_same_user(self):
        ...
        review1 = Review(text="Excellent massage therapy session", rating=5, user=self.user)
        review2 = Review(text="Great reflexology treatment", rating=4, user=self.user)
        review3 = Review(text="Amazing aromatherapy experience", rating=5, user=self.user)
