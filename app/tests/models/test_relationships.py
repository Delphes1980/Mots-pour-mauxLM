...
class TestAllRelations(unittest.TestCase):
    def setUp(self):
    ...
    def test_user_with_reviews_and_appointments(self):
        ...
        review = Review(text="Excellent service!", rating=5, user=self.user)
        appointment = Appointment(subject="Massage suédois", 
                                message="Nouveau rendez-vous", user=self.user)
    ...
    def test_multiple_users_multiple_entities(self):
        ...
        review1_u1 = Review(text="Great massage", rating=5, user=self.user)
        review2_u1 = Review(text="Good reflexology", rating=4, user=self.user)
        appointment1_u1 = Appointment(subject="Aromathérapie", 
                                    message="Séance détente", user=self.user)
        
        # User 2: 1 avis + 2 rendez-vous
        review1_u2 = Review(text="Amazing experience", rating=5, user=self.user2)
        appointment1_u2 = Appointment(subject="Massage thérapeutique", 
                                    message="Soulager douleurs", user=self.user2)
        appointment2_u2 = Appointment(subject="Réflexologie", 
                                    message="Bien-être", user=self.user2)
    ...
    def test_entity_isolation_between_users(self):
        ...
        review_u1 = Review(text="User 1 review", rating=5, user=self.user)
        review_u2 = Review(text="User 2 review", rating=4, user=self.user2)
        
        appointment_u1 = Appointment(subject="User 1 appointment", 
                                   message="Message 1", user=self.user)
        appointment_u2 = Appointment(subject="User 2 appointment", 
                                   message="Message 2", user=self.user2)
    ...
    def test_user_business_scenario(self):
        ...
        first_appointment = Appointment(subject="Massage découverte", 
                                      message="Premier essai", user=self.user)
    ...
    def test_data_consistency_across_relations(self):
        ...
        review = Review(text="Cohérence test", rating=5, user=self.user)
        appointment = Appointment(subject="Test cohérence", 
                                message="Message test", user=self.user)
    ...
    def test_reassign_entity_to_another_user(self):
        review = Review(text="Initial", rating=5, user=self.user)
        self.assertEqual(review.user, self.user)
    ...
    def test_reassigning_entity_updates_relationships(self):
        ...
        review = Review(text="Initial review", rating=5, user=self.user)
        ...
        appointment = Appointment(subject="Initial appointment", message="Test", user=self.user)
        self.assertIn(appointment, self.user.appointments)
        
        appointment.user = self.user2
        self.assertNotIn(appointment, self.user.appointments)
    ...
    def test_user_reviews_and_appointments_are_separate_lists(self):
        ...
        review = Review(text="Test distinct lists", rating=5, user=self.user)
        appointment = Appointment(subject="Test distinct lists", message="Test", user=self.user)
