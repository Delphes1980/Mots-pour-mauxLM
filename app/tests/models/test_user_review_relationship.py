...
class TestUserReviewRelations(unittest.TestCase):
    def setUp(self):
    ...
    def test_user_multiple_reviews(self):
        ...
        review1 = Review(text="Great massage!", rating=5, user=self.user)
        review2 = Review(text="Good reflexology session", rating=4, user=self.user)
    ...
    def test_review_belongs_to_one_user(self):
        ...
        review = Review(text="Nice service", rating=4, user=self.user)
    ...
    def test_different_users_different_reviews(self):
        ...
        review1 = Review(text="Amazing experience", rating=5, user=self.user)
        review2 = Review(text="Very relaxing", rating=4, user=self.user2)
    ...
    # Testez si l'ajout d'un avis à la liste de l'utilisateur met à jour la relation inverse
        review = Review(text="Test back_populates", rating=5, user=self.user)
        self.assertIn(review, self.user.reviews)
        self.assertEqual(review.user, self.user)

    # Testez la même chose pour les rendez-vous
        appointment = Appointment(subject="Test back_populates", message="Test", user=self.user)
